...
YOVariant::YOVariant(const std::string &name, YOValue val) : m_name(std::move(name)), m_value(std::move(val)) {
    std::cout << "TYPE " << m_value.index() << std::endl;
}
...
YOVariant::YOVariant(const char *data, size_t size)
        {
    unpack(data, size);
}
...
YOVariant& YOVariant::get(const std::string &key) {
    std::get<YOMap>(m_value)[key].m_name = key;
...
std::ostream& operator<<(std::ostream &os, const YOValue &v) {
    std::visit([&v, &os](auto &value) { os << "[" << YOValue_type_name(v.index()) << "] " << value << "\n";}, v);
    return os;
}
